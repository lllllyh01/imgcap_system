...
def main():
    ...
    for i in range(6, 10):
        ...
        if i < 9:
            for j in range(i):
                # train_set.append(CaptionDataset(data_folder, data_name, max_len, max_tag_len, split='TRAIN', prefix='TRAIN', fold_id=j, transform=transforms.Compose([normalize])))
                # train_set.append(CaptionDataset(data_folder, data_name, max_len, max_tag_len, split='TRAIN', prefix='REST', fold_id=j, transform=transforms.Compose([normalize])))
        ...
        #     batch_size=batch_size, shuffle=True, num_workers=workers, pin_memory=True)
        # print("train_data len:", train_loader.__len__())
        # print("long cap num:", train_set.long_cap_num)
        # input("Please press the enter to proceed")
        # print("val_data len:", val_loader.__len__())
...
def train(train_loader, encoder, decoder, criterion,criterion1, encoder_optimizer, decoder_optimizer, epoch):
    ...
    for i, (imgs, caps, caplens,tags,taglens,tags_yn) in enumerate(train_loader):
        ...
        # if drop:
        #     continue
        # print("tag_lens:", taglens) # (batch_size, 1), 每行包括一个数字，表示该数据tags长度（为1的）
        # print(sum(taglens))
        # print("tags shape:", tags.shape) #(batch_size(16), 52(含'start''end'))
        '''
        for batch in tags:
        ...
        # print("caps shape:", caps.shape) # (batch_size, 52)
        # print("tag_yn shape:", tags_yn.shape) #(batch_size, 209) (209 is the size of tagmap)
        # taglen=10
        ...
        #     print(f'batch{i}-heads_weight_i:{heads_weight_i}')
