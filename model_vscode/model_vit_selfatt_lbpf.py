...
class Encoder(nn.Module):
    ...
    def __init__(self, encoder_dim, decoder_dim, encoded_image_size=14):
        ...
        # self.linear2=nn.Linear(encoder_dim, decoder_dim)
        ...
        # self.linear1=nn.Sequential(nn.Linear(2048,512),nn.ReLU(inplace=True))
        # self.linear2=nn.Sequential(nn.Linear(512,512),nn.ReLU(inplace=True))
        self.global_cap=nn.LSTM(encoder_dim,decoder_dim,batch_first=True,num_layers=1)
        # self.embed=nn.Sequential(nn.Linear(2048,512),nn.ReLU(inplace=True))
...
#         self.linear=nn.Linear(encoder_dim,tag_size)
#         self.softmax=nn.Softmax(dim=1)
...
#         # pix=encoder_out.size(1)
#         # encode_size=int(math.sqrt(pix))
#         # print(batch_size)
#         # print(encoder_dim)
#         # print(encode_size)
...
#         out=self.pool(encoder_out) #
#         # out=out.flatten(1)
#         # print(out.shape, encoder_dim)
#         out=self.linear(out)
#         out=self.softmax(out)
#         print("mlp out shape:", out.shape)
#         return out
...
#         self.v_att=nn.Linear(encoder_dim,attention_dim)
#         self.t_att=nn.Linear(decoder_dim,attention_dim)
#         self.h_att=nn.Linear(decoder_dim,attention_dim)
#         self.softmax=nn.Softmax(dim=1)
#         self.relu=nn.ReLU()
#         self.vatt=nn.Linear(attention_dim,attention_dim)
#         self.aatt=nn.Linear(attention_dim,attention_dim)

#         self.fc=nn.Linear(attention_dim,decoder_dim)

#         self.ff=nn.Linear(attention_dim,1)
#         self.ff1=nn.Linear(attention_dim,1)

#     def forward(self, encoder_out, decoder_hidden,tag_embedding):
#         """
#         Forward propagation.
...
#         v_att=self.v_att(encoder_out)  
#         t_att=self.t_att(tag_embedding) #(batch_size,tag_len,att_dim)
#         # print(t_att.shape)
#         h_att=self.h_att(decoder_hidden)  #(batch_size,att_dim)
#         a_vn=self.ff1(self.relu(v_att+h_att.unsqueeze(1))).squeeze(2)
#         a_vn=self.softmax(a_vn)
#         # a_vn=self.softmax(self.vatt(tanh(v_att+h_att))) #图像特征
#         # print(h_att.shape)
#         a_am=self.ff(self.relu(t_att+h_att.unsqueeze(1))).squeeze(2) ##(batch_size,tag_len)
       
#         a_am=self.softmax(a_am) #标签特征  #(batch_size,tag_len,att_dim)
#         # print(a_am.shape)

#         # attention_weighted_encoding = (encoder_out * alpha.unsqueeze(2)).sum(dim=1)  # (batch_size, encoder_dim)
#         v_f=(a_vn.unsqueeze(2)*encoder_out).sum(dim=1)
#         # v_f=a_vn* encoder_out
#         a_f=(a_am.unsqueeze(2)* tag_embedding).sum(dim=1)
#         # print(a_f.shape)
#         ctx=self.fc(v_f+a_f)
#         # print(ctx.shape)
#         return ctx
...
class Attention(nn.Module):
    ...
    def forward(self, encoder_out, decoder_hidden):
        ...
        ## w_kt = self.w_kt(tag_embedding)
        ## w_vt = self.w_vt(tag_embedding)
        # print("shape of encoder_out:", encoder_out.shape) # (batch_size_t, 196, encoder_dim(512))
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths,tags_yn,rev_tag_map,word_map,hc):
        ...
        ## encoded_tag=self.tag_convert(mlp_out,rev_tag_map,word_map) # (batch_size, tag_len+2('start''end')), encoded_tag为标签列表（用tag_map中的数字表示）
        # print("encoded_tag shape:", encoded_tag.shape)
        # print("encoded_tag:", encoded_tag)
        # Flatten image
        ...
        ## tagembedding=self.tagembedding(encoded_tag)  #(batch_size,max_tag_length,enbed_dim)
        ## tags_embedding=self.tagembedding(tags_yn)
        # tagembedding=self.embedding(encoded_tag)
        
        # Initialize LSTM state
        h1,c1=self.init_hidden_state(batch_size)
        h2,c2=self.init_hidden_state(batch_size)

        ## _,h_c_tag=self.global_tag(tagembedding)
        ## h1,c1=h_c_tag
        ## h1=h1.squeeze(0)
        ## c1=c1.squeeze(0)
